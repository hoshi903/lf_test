...
で、公開後(に息抜きの麻雀でボロ負けし)、色々弄ってNuxtもなんとなくわかったので、1からNuxtと[markdown-it](https://github.com/markdown-it/markdown-it)で動くblogを自作する方向に進みました。
...
<small style="color: lightgray;">もしかしたらNuxtを選択すべきじゃないアレかもしれないが**応用から始めて基本を学ぶ**全飛ばし逆走学習法なので容赦してほしい</small>

　<br>

因みに生成した静的ファイル群は新たに借りた容量20倍(1GBまで)のhoshi.s1007.xrea.comに置いてます。

これまで容量50MBではMP3も置けない！みたいなネタを繰り出してきたけど今のothersは2MBくらいで、これならページ全体をNuxtにしても全然（ｒｙ
...
手順は割愛してothersで試した結果です。([github.com/hoshi903/lf_test](https://github.com/hoshi903/lf_test) -> [903.netlify.app](https://903.netlify.app/))

　<br>

Nuxtで作ったこのothersも「ビルドしてFFFTP開いてファイル選んでアップロード」せずとも「git push」して待っていれば変わってる！という経験をみなさまにも一度はお試しいただきたい（
...
(HTMLの外部ファイルに過ぎない)JavaScriptにファイル構造？というのが謎でアレでしたが、完成した今は普通のバックエンド開発になったのかなというのが感想です。

とりあえず、微妙に余ってたサブドメインも有効利用したし(hoshi.903.ch)、デザインもちゃんと共通にできたし、構築はだいたい目標通りです。

　<br>

これ以降も、自作ソフトやModの記事、[エアコミケ98](/blog/20200505_c98/)やVket4の告知の詳細、画像がたくさんあるページ(?)などを追加していくと思います。

もちろん必要そうになってきたらタグ機能とか一覧にソート機能とか
...
**さぁ、Webにありがちな新たなる小手先の技術、登場です！！(ブチ切れ)**

Markdown-it v10.0.0 で aタグに常に `target="_blank" rel="noopener"` 属性が付くように改造します。

　<br>

`/markdown-it/lib/rules_inline/link.js` を開いて、137行目の
...
付録のチートシートに使い分け方を載せておきます

　<br>

ついでに応用して `/markdown-it/lib/rules_inline/image.js` を開いて、140行目の
...
imgタグも[lazysizes](https://github.com/aFarkas/lazysizes)の基本形にしました。

　<br>

改変するならいくつやっても同じことなので周辺のjsファイルを漁って他のタグもじゃんじゃんカスタマイズしよう！（
...
常に `target="_blank" rel="noopener"` が付くように改変済、以下のように使い分ける予定
`n-link`タグは前のページスクロール状態がリセットされない(SPAあるある)ので注意
...
今の所なし、~~入れても動かなくてmarkdown-it本体を改造するくらいやし...~~
